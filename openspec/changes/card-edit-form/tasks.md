...
- [ ] Тестирование через API: `PUT /api/courses/1/cards/123` с
  `{ front: "test", back: "test", resetProgress: true }` сбрасывает прогресс
- [ ] Response возвращает обновлённую карточку с `state = New`, `reps = 0`, etc.
...
- [ ] `scrollToCardWithBounce()` вызывается и прокручивает к карточке
- [ ] Анимация `bounce-in-bck` отображается корректно
- [ ] После завершения анимации класс `anim-bounce-in-bck` удаляется
- [ ] Если карточка не найдена, warning в консоли
...
- [ ] После сохранения редактирования список прокручивается к карточке
- [ ] Карточка показывает bounce-анимацию
- [ ] Прогресс карточки сброшен (state = New, reps = 0)
...
- [ ] После добавления карточки список прокручивается к ней
...
1. Редактирование карточки:
   ...
   - [ ] Модалка открывается с данными карточки
   - [ ] Изменить `front` и `back`
   - [ ] Нажать "Сохранить"
   - [ ] Модалка закрывается
   - [ ] Список прокручивается к отредактированной карточке
   - [ ] Карточка показывает bounce-анимацию
   - [ ] Прогресс карточки сброшен (state = New, reps = 0)

2. Добавление карточки:
   - [ ] Заполнить форму добавления
   - [ ] Нажать "Добавить карточку"
   - [ ] Список прокручивается к добавленной карточке
   - [ ] Карточка показывает bounce-анимацию
...
1. Редактирование карточки:
   - [ ] Открыть мобильную панель карточек
   - [ ] Нажать "Редактировать" на карточке
   - [ ] Модалка открывается
   - [ ] Нажать "Сохранить"
   - [ ] Список прокручивается к отредактированной карточке
...
2. Добавление карточки:
   - [ ] Заполнить форму добавления
   - [ ] Нажать "Добавить карточку"
   - [ ] Список прокручивается к добавленной карточке (можно в панели, можно на странице)
   - [ ] Карточка показывает bounce-анимацию
...
1. [ ] Прокрутка к карточке, которая не существует → warning в консоли, нет ошибки
2. [ ] Анимация bounce при `prefers-reduced-motion: reduce` → анимация не воспроизводится
3. [ ] Редактирование последней карточки в длинном списке → прокрутка работает
4. [ ] Добавление карточки в пустой список → прокрутка работает (или не происходит, если список был пуст)
...
- [ ] Changelog содержит все основные изменения
