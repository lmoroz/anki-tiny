# Capability: Real-Time Statistics Streaming

## Overview

Система обновления статистики в реальном времени через Server-Sent Events (SSE), заменяющая polling-подход. Бэкенд интеллектуально планирует обновления на моменты, когда карточки становятся доступны для тренировки, и немедленно отправляет изменения при модификации данных.
