...
* **Target Directory:** All planning files (`Task.md`, `Walkthrough.md`, `Implementation_Plan.md`) must be physically
  located and updated in the `./docs/` folder in the project root.
* **Sync Policy:** Ignore your internal profile/hidden directories. Any change to the plan or task status must be
  immediately written to the files in `./docs/`.
...
2. **Update History:**
    * **Mandatory:** Append a summary of this session's work to `./docs/Changelog.md` (create the file if it doesn't
      exist).
...
2. **Integration:** Incorporate these manual changes into `Walkthrough.md` so the documentation remains consistent with
...
1. **Staging:** Stage (`git add`) all changed code files (including lint fixes), updated `package.json` files, and the
   `./docs/` directory.
