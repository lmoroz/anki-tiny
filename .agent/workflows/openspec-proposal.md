...
- Refer to `openspec/AGENTS.md` (located inside the `openspec/` directory—run `ls openspec` or
  `npx @fission-ai/openspec update` if you don't see it) if you need additional OpenSpec conventions or clarifications.
...
1. Review `openspec/project.md`, run `npx @fission-ai/openspec list` and `npx @fission-ai/openspec list --specs`, and
   inspect related code or docs (
...
7. Validate with `npx @fission-ai/openspec validate <id> --strict` and resolve every issue before sharing the proposal.

**Reference**

- Use `npx @fission-ai/openspec show <id> --json --deltas-only` or `npx @fission-ai/openspec show <spec> --type spec` to
  inspect details when validation
  fails.
