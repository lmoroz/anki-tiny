...
4. Настройки индивидуальные для каждого курса, по-умолчанию, берутся из общих,
   но можно отредактировать для каждого курса отдельно
...
8. система, используемая для запоминания — user customized FSRS (используем библиотеку https://github.com/open-spaced-repetition/ts-fsrs ):
   пользователь хочет адаптировать интервалы под себя,
   допустим, сделать самый первый, короткий интервал — не сутки, а 4 часа
...
2. Установите зависимости для всех workspaces (один раз из корня проекта):
   ...
   > **⚠️ Важно**: После установки зависимостей автоматически выполнится `postinstall` скрипт в backend, который запустит `electron-rebuild` для сборки нативного модуля `better-sqlite3`. Это может занять 1-2
   минуты при первой установке.
